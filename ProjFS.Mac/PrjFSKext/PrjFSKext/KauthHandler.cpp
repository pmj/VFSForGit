    ...
    uintptr_t       arg3)
{
    ...
    {
        functionSample.setProbe(Probe_VnodeOp_EarlyOut);
    ...
    pid = GetPid(context);
    functionSample.takeSplitSample(Probe_VnodeOpSplit_EarlyChecks);

    currentVnodeFileFlags = ReadVNodeFileFlags(currentVnode, context);

    functionSample.takeSplitSample(Probe_VnodeOpSplit_ReadFileFlagsSplit);
    ...
    if (!FileFlagsBitIsSet(currentVnodeFileFlags, FileFlags_IsInVirtualizationRoot))
    {
        ...
        functionSample.setProbe(Probe_VnodeOp_NoVirtualizationRootFlag);
        
        cacheRootIndex = VNodeCache_FindVnodeVirtualizationRoot(currentVnode, parentVnode, -1);
        functionSample.takeSplitSample(Probe_VnodeOpSplit_NoFlagCacheLookup);

        if (cacheRootIndex != -1)
        {
            KextLog_FileNote(
                currentVnode,
                "HandleVnodeOperation: PROVIDER DETECTION MISMATCH: no virtualisation root file flag, root index %d according to vnode cache.",
                cacheRootIndex);
        }
    ...
    functionSample.takeSplitSample(Probe_VnodeOpSplit_FindRootStandard);

    cacheRootIndex = VNodeCache_FindVnodeVirtualizationRoot(currentVnode, parentVnode, rootIndex);

    functionSample.takeSplitSample(Probe_VnodeOpSplit_FindRootCached);

    if (cacheRootIndex == -1)
    {
        KextLog_FileNote(
            currentVnode,
            "HandleVnodeOperation: PROVIDER DETECTION MISMATCH: virtualisation root file flag present, root index %d according to vnode cache. Is the provider offline?",
            cacheRootIndex);
    }
    ...
    {
        functionSample.setProbe(Probe_VnodeOp_EmptyFlag);
        ...
        if (IsFileSystemCrawler(procname))
        {
            ...
            functionSample.setProbe(Probe_VnodeOp_DenyCrawler);
            goto CleanupAndReturn;
        }
    }
    ...
    {
        functionSample.setProbe(Probe_VnodeOp_Offline);
    ...
    {
        functionSample.setProbe(Probe_VnodeOp_Provider);
        kauthResult = KAUTH_RESULT_DEFER;
        goto CleanupAndReturn;
    }
    ...
    if (VDIR == vnodeType)
    {
        ...
        else
        {
            if (FileFlagsBitIsSet(currentVnodeFileFlags, FileFlags_IsEmpty))
            {
                KextLog_FileNote(currentVnode, "HandleVnodeOperation - action 0x%x by process %u (%s) on empty directory not handled.", action, pid, procname);
            }
        }
    }
    else
    {
        ...
        else
        {
            if (FileFlagsBitIsSet(currentVnodeFileFlags, FileFlags_IsEmpty))
            {
                KextLog_FileNote(currentVnode, "HandleVnodeOperation - action 0x%x by process %u (%s) on empty file not handled\n", action, pid, procname);
            }
        }
    }
...
#define YESNO_STRING(b) ((b) ? "yes" : "\033[31;1mno\033[0m")

    KextLog_Info("message %6d response: %s, success: %s (file %s)",
        nextMessageId, YESNO_STRING(message.receivedResponse), YESNO_STRING(message.response == MessageType_Response_Success), relativePath);
