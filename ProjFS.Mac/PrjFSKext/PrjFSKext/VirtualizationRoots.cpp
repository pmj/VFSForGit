...
VirtualizationRoot* VirtualizationRoots_FindForVnode(vnode_t vnode)
{
    ...
    while (nullptr == root && NULLVP != vnode && !vnode_isvroot(vnode))
    {
        ...
        int16_t rootIndex = VirtualizationRoots_FindRootWithVnode(vnode, context);
        if (rootIndex >= 0)
        {
...
void LogFileImpl(const char* fmt, vnode_t file, args... a)
{
    char vnodePath[PrjFSMaxPath] = "";
    int vnodePathLength = PrjFSMaxPath;
    vn_getpath(file, vnodePath, &vnodePathLength);
    kprintf(fmt, a..., vnodePath);
}

#define LogFile(fmt, ...) LogFileImpl("\033[31;1mNote:\033[0m " fmt "\n", ##__VA_ARGS__)

int16_t VirtualizationRoots_FindRootWithVnode(vnode_t vnode, vfs_context_t context)
{
    ...
    if (rootIndex < 0)
    {
        ...
        if (xattrResult.error == 0)
        {
            ...
            char* path = static_cast<char*>(Memory_Alloc(PrjFSMaxPath));
            ...
            KextLog_Info(
                "Directory root xattr read result: %d (xattr size %lu) on directory '%s' succeeded, vnode identified as virtualisation root %d",
                xattrResult.error, xattrResult.size, path, rootIndex);
            Memory_Free(path, PrjFSMaxPath);
        }
    }
...
static int16_t FindRootForVnode_Locked(vnode_t vnode, uint32_t vid, VnodeFsidInode fileId)
{
    for (int16_t i = 0; i < MaxVirtualizationRoots; ++i)
    {
        ...
        //KextLog_Info("FindRootForVnode_Locked: root %u in use, vnode %p, vid %u, fsid 0x%08x:%08x, inode 0x%llx",
        //    i, rootEntry.rootVNode, rootEntry.rootVNodeVid, rootEntry.rootFsid.val[0], rootEntry.rootFsid.val[1], rootEntry.rootInode);
        if (rootEntry.rootVNode == vnode && rootEntry.rootVNodeVid == vid)
        {
            //KextLog_Note("Matching vnode/vid for index %d", i);
        ...
        {
            KextLog_Note("Matching fsid/inode for index %d, updating vnode/vid", i);
        ...
        else
        {
            ...
            RWLock_AcquireExclusive(s_rwLock);
            {
                ...
                if (rootIndex >= 0)
                {
                    ...
                    VNodeCache_SetRootIndexForVnode_Locked(virtualizationRootVNode, rootVid, rootIndex);
                }
                else
                {
                    ...
                    if (rootIndex >= 0)
                    {
                        ...
                        int count = 0;
                        struct rootNodes rn[MaxVirtualizationRoots];
                        for(int i = 0; i < MaxVirtualizationRoots; i++)
                        {
                        
                            if(NULLVP != s_virtualizationRoots[i].rootVNode)
                            {
                                rn[count].vnode = s_virtualizationRoots[i].rootVNode;
                                rn[count].providerIndex = i;
                                count++;
                            }
                        }
