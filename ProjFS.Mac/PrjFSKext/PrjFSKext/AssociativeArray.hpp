...
public:
    ...
    {
        return this->NextGrowCapacity() * sizeof(VALUE);
    }
    ...
    ResizeResult ReserveCapacity(uint32_t newCapacity, void* memory, uint32_t memorySize)
    {
        ...
        {
            VALUE* moved = new(memoryPosition) VALUE(this->data[i]);
            (void)moved;
            this->data[i].~VALUE();
        }
    ...
    VALUE* TryInsert(const VALUE& newItem)
    {
    ...
    VALUE* FindOrTryInsert(KEY key, const VALUE& newItem)
    {
        VALUE* value = this->Find(key);
        if (nullptr == value)
        {
            value = this->TryInsert(newItem);
        }
        return value;
    }
    ...
    VALUE* FindOrInsertLockedAllowResize(KEY key, const VALUE& newItem, RWLock& lock)
    {
        while (true)
        {
            VALUE* value = this->FindOrTryInsert(key, newItem);
            if (nullptr != value)
            {
                return value;
            }
            ...
            if (nullptr == reservedMemory)
            {
                ...
                return this->FindOrTryInsert(key, newItem);
            }
