...
- (void)testRegisterProviderForPath_NonDirectoryPath
{
    ...
    VnodePointer vnode = vnode::Create(self->testMountPoint, path);
...
- (void)testRegisterProviderForPath_DisallowedFileSystem
{
    ...
    fatFsid.val[1]++;
    MountPointer fatMount(mount::Create("msdos", fatFsid, rand()));
    
    VnodePointer vnode = vnode::Create(fatMount, path);
...
- (void)testRegisterProviderForPath_GetPathError
{
    ...
    VnodePointer vnode = vnode::Create(self->testMountPoint, path, VDIR);
...
- (void)testRegisterProviderForPath_ExistingRecycledRoot
{
    ...
    VnodePointer oldVnode = vnode::Create(self->testMountPoint, path, VDIR);
    ...
    VnodePointer newVnode = vnode::Create(self->testMountPoint, path, VDIR, oldVnode->GetInode());
